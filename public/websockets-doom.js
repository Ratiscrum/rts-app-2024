...
(() => {
  ...
  function loadPackage(metadata) {
    ...
    var PACKAGE_NAME = 'websockets-doom.data';
    var REMOTE_PACKAGE_BASE = 'websockets-doom.data';
    if (
    ...
    function runWithFS() {
      ...
      function processPackageData(arrayBuffer) {
        ...
        Module['removeRunDependency']('datafile_websockets-doom.data');
      }
      Module['addRunDependency']('datafile_websockets-doom.data');
...
var ASM_CONSTS = {
  ...
  594426: ($0, $1) => {
    ...
    for (var c = 0; c < numChannels; ++c) {
      ...
      for (var j = 0; j < $1; ++j) {
        channelData[j] = SAFE_HEAP_LOAD_D(
          (($0 + ((j * numChannels + c) << 2)) >> 2) * 4,
          4,
          0,
        );
      }
...
var SYSCALLS = {
  ...
  doStat(func, path, buf) {
    ...
    (tempI64 = [
      ...
      SAFE_HEAP_STORE(((buf + 24) >> 2) * 4, tempI64[0], 4),
      SAFE_HEAP_STORE(((buf + 28) >> 2) * 4, tempI64[1], 4);
    ...
    (tempI64 = [
      ...
      SAFE_HEAP_STORE(((buf + 40) >> 2) * 4, tempI64[0], 4),
      SAFE_HEAP_STORE(((buf + 44) >> 2) * 4, tempI64[1], 4);
    ...
    (tempI64 = [
      ...
      SAFE_HEAP_STORE(((buf + 56) >> 2) * 4, tempI64[0], 4),
      SAFE_HEAP_STORE(((buf + 60) >> 2) * 4, tempI64[1], 4);
    ...
    (tempI64 = [
      ...
      SAFE_HEAP_STORE(((buf + 72) >> 2) * 4, tempI64[0], 4),
      SAFE_HEAP_STORE(((buf + 76) >> 2) * 4, tempI64[1], 4);
    ...
    (tempI64 = [
      ...
      SAFE_HEAP_STORE(((buf + 88) >> 2) * 4, tempI64[0], 4),
      SAFE_HEAP_STORE(((buf + 92) >> 2) * 4, tempI64[1], 4);
    return 0;
  },
  ...
  try {
    ...
    while (idx < stream.getdents.length && pos + struct_size <= count) {
      ...
      (tempI64 = [
        ...
        SAFE_HEAP_STORE(((dirp + pos) >> 2) * 4, tempI64[0], 4),
        SAFE_HEAP_STORE(((dirp + pos + 4) >> 2) * 4, tempI64[1], 4);
      (tempI64 = [
        ((idx + 1) * struct_size) >>> 0,
        ...
        SAFE_HEAP_STORE(((dirp + pos + 8) >> 2) * 4, tempI64[0], 4),
        SAFE_HEAP_STORE(((dirp + pos + 12) >> 2) * 4, tempI64[1], 4);
  ...
  chooseConfig(display, attribList, config, config_size, numConfigs) {
    ...
    if (attribList) {
      for (;;) {
        ...
        if (param == 12321) {
          /*EGL_ALPHA_SIZE*/ var alphaSize = SAFE_HEAP_LOAD(
            ((attribList + 4) >> 2) * 4,
            4,
            0,
          );
          ...
          /*EGL_DEPTH_SIZE*/ var depthSize = SAFE_HEAP_LOAD(
            ((attribList + 4) >> 2) * 4,
            4,
            0,
          );
          ...
          /*EGL_STENCIL_SIZE*/ var stencilSize = SAFE_HEAP_LOAD(
            ((attribList + 4) >> 2) * 4,
            4,
            0,
          );
          ...
          /*EGL_SAMPLES*/ var samples = SAFE_HEAP_LOAD(
            ((attribList + 4) >> 2) * 4,
            4,
            0,
          );
          ...
          /*EGL_SAMPLE_BUFFERS*/ var samples = SAFE_HEAP_LOAD(
            ((attribList + 4) >> 2) * 4,
            4,
            0,
          );
          ...
          /*EGL_CONTEXT_PRIORITY_LEVEL_IMG*/ var requestedPriority =
            SAFE_HEAP_LOAD(((attribList + 4) >> 2) * 4, 4, 0);
...
var GL = {
  ...
  getSource: (shader, count, string, length) => {
    ...
    for (var i = 0; i < count; ++i) {
      var len = length
        ? SAFE_HEAP_LOAD(((length + i * 4) >> 2) * 4, 4, 1)
        : undefined;
      source += UTF8ToString(
        SAFE_HEAP_LOAD(((string + i * 4) >> 2) * 4, 4, 1),
        len,
      );
    }
    return source;
...
var _eglCreateContext = (display, config, hmm, contextAttribs) => {
  ...
  for (;;) {
    ...
    if (param == 12440) {
      /*EGL_CONTEXT_CLIENT_VERSION*/ glesContextVersion = SAFE_HEAP_LOAD(
        ((contextAttribs + 4) >> 2) * 4,
        4,
        0,
      );
    } else if (param == 12344) {
...
/* EGL_NO_CONTEXT */ var _eglCreateWindowSurface = (
  display,
  config,
  win,
  attrib_list,
...
/* Magic ID for Emscripten 'default surface' */ var _eglDestroyContext = (
  display,
  context,
) => {
  if (display != 62e3) {
...
/* Magic ID for Emscripten 'default surface' */ var _eglGetConfigAttrib = (
  display,
  config,
  attribute,
  value,
  ...
  /* EGL_SUCCESS */ switch (attribute) {
    ...
    case 12325:
      SAFE_HEAP_STORE(
        (value >> 2) * 4,
        EGL.contextAttributes.depth ? 24 : 0,
        4,
      );
      return 1;
...
var _eglSwapBuffers = (dpy, surface) => {
  if (!EGL.defaultDisplayInitialized) {
    ...
    /* EGL_NOT_INITIALIZED */ EGL.setErrorCode(12290);
  } else if (Module.ctx.isContextLost()) {
    /* EGL_BAD_ACCESS */ EGL.setErrorCode(12302);
...
var readEmAsmArgs = (sigPtr, buf) => {
  ...
  while ((ch = SAFE_HEAP_LOAD(sigPtr++, 1, 1))) {
    ...
    readEmAsmArgsArray.push(
      ch == 112
        ? SAFE_HEAP_LOAD((buf >> 2) * 4, 4, 1)
        : ch == 105
          ? SAFE_HEAP_LOAD((buf >> 2) * 4, 4, 0)
          : SAFE_HEAP_LOAD_D((buf >> 3) * 8, 8, 0),
    );
    buf += wide ? 8 : 4;
  }
...
var getCanvasElementSize = (target) => {
  ...
  var size = [
    SAFE_HEAP_LOAD((w >> 2) * 4, 4, 0),
    SAFE_HEAP_LOAD((h >> 2) * 4, 4, 0),
  ];
  stackRestore(sp);
  return size;
};
  ...
  for (var i = 0; i < n; i++) {
    ...
    if (!query) {
      ...
      /* GL_INVALID_OPERATION */ while (i < n)
        SAFE_HEAP_STORE(((ids + i++ * 4) >> 2) * 4, 0, 4);
      return;
    }
...
var readI53FromI64 = (ptr) =>
  SAFE_HEAP_LOAD((ptr >> 2) * 4, 4, 1) +
  SAFE_HEAP_LOAD(((ptr + 4) >> 2) * 4, 4, 0) * 4294967296;

var readI53FromU64 = (ptr) =>
  SAFE_HEAP_LOAD((ptr >> 2) * 4, 4, 1) +
  SAFE_HEAP_LOAD(((ptr + 4) >> 2) * 4, 4, 1) * 4294967296;
...
var writeI53ToI64 = (ptr, num) => {
  ...
  if (deserialized != num)
    warnOnce(
      `writeI53ToI64() out of range: serialized JS Number ${num} to Wasm heap as bytes lo=${ptrToString(SAFE_HEAP_LOAD(offset * 4, 4, 1))}, hi=${ptrToString(SAFE_HEAP_LOAD((offset + 1) * 4, 4, 1))}, which deserializes back to ${deserialized} instead!`,
    );
};
...
/** @suppress{checkTypes} */ var emscriptenWebGLGetVertexAttrib = (
  ...
  if (pname == 34975) {
    /*VERTEX_ATTRIB_ARRAY_BUFFER_BINDING*/ SAFE_HEAP_STORE(
      (params >> 2) * 4,
      data && data['name'],
      4,
    );
...
var _emscripten_request_fullscreen_strategy = (
  ...
  var strategy = {
    ...
    canvasResizedCallback: SAFE_HEAP_LOAD(
      ((fullscreenStrategy + 12) >> 2) * 4,
      4,
      0,
    ),
    canvasResizedCallbackUserData: SAFE_HEAP_LOAD(
      ((fullscreenStrategy + 16) >> 2) * 4,
      4,
      0,
    ),
  };
...
var _emscripten_websocket_set_onclose_callback_on_thread = (
  ...
  socket.onclose = function (e) {
    SAFE_HEAP_STORE(((eventPtr + 4) >> 2) * 4, e.wasClean, 4),
      SAFE_HEAP_STORE(((eventPtr + 4) >> 1) * 2, e.code, 2),
...
var _emscripten_websocket_set_onmessage_callback_on_thread = (
  ...
  socket.onmessage = function (e) {
    ...
    SAFE_HEAP_STORE(((eventPtr + 4) >> 2) * 4, buf, 4),
      SAFE_HEAP_STORE(((eventPtr + 8) >> 2) * 4, len, 4),
      SAFE_HEAP_STORE(((eventPtr + 12) >> 2) * 4, isText, 4),
      ((a1, a2, a3) => dynCall_iiii(callbackFunc, a1, a2, a3))(
        0,
        /*TODO*/ eventPtr,
        userData,
...
function _fd_fdstat_get(fd, pbuf) {
  try {
    ...
    (tempI64 = [
      ...
      SAFE_HEAP_STORE(((pbuf + 8) >> 2) * 4, tempI64[0], 4),
      SAFE_HEAP_STORE(((pbuf + 12) >> 2) * 4, tempI64[1], 4);
    (tempI64 = [
      rightsInheriting >>> 0,
      ...
      SAFE_HEAP_STORE(((pbuf + 16) >> 2) * 4, tempI64[0], 4),
      SAFE_HEAP_STORE(((pbuf + 20) >> 2) * 4, tempI64[1], 4);
    return 0;
  } catch (e) {
    if (typeof FS == 'undefined' || !(e.name === 'ErrnoError')) throw e;
    return e.errno;
...
function _fd_seek(fd, offset_low, offset_high, whence, newOffset) {
  ...
  try {
    ...
    (tempI64 = [
      ...
      SAFE_HEAP_STORE((newOffset >> 2) * 4, tempI64[0], 4),
      SAFE_HEAP_STORE(((newOffset + 4) >> 2) * 4, tempI64[1], 4);
...
var wasmImports = {
  ...
  /** @export */ emscripten_websocket_get_ready_state:
    _emscripten_websocket_get_ready_state,
  /** @export */ emscripten_websocket_new: _emscripten_websocket_new,
  /** @export */ emscripten_websocket_send_binary:
    _emscripten_websocket_send_binary,
  /** @export */ emscripten_websocket_set_onclose_callback_on_thread:
    _emscripten_websocket_set_onclose_callback_on_thread,
  /** @export */ emscripten_websocket_set_onerror_callback_on_thread:
    _emscripten_websocket_set_onerror_callback_on_thread,
  /** @export */ emscripten_websocket_set_onmessage_callback_on_thread:
    _emscripten_websocket_set_onmessage_callback_on_thread,
  /** @export */ emscripten_websocket_set_onopen_callback_on_thread:
...
var _get_total_kills = (Module['_get_total_kills'] = createExportWrapper(
  'get_total_kills',
  0,
));
...
var dynCall_iid = (Module['dynCall_iid'] = createExportWrapper(
  'dynCall_iid',
  3,
));
...
var dynCall_iiiiiiiiiiiiiiff = (Module['dynCall_iiiiiiiiiiiiiiff'] =
  createExportWrapper('dynCall_iiiiiiiiiiiiiiff', 16));
...
var dynCall_iiiiiidiiff = (Module['dynCall_iiiiiidiiff'] = createExportWrapper(
  'dynCall_iiiiiidiiff',
  11,
));
